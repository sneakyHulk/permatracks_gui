...
class AfterReturnTimeMeasure {
	std::chrono::time_point<std::chrono::system_clock, std::chrono::nanoseconds> const t0;
	...
	void init() {
		...
		if (!file.is_open()) {
			...
			file = std::ofstream(output_folder / (filename + ".csv"));
   ...
   public:
	explicit AfterReturnTimeMeasure(std::chrono::time_point<std::chrono::system_clock, std::chrono::nanoseconds> const t0, std::source_location const location = std::source_location::current()) : t0(t0), location(location) { init(); }
